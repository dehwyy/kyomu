...
impl RuntimeConfig {
  pub fn new() -> Self {
      ...
      for arg in args {
        if arg.starts_with("--") {
          // "--fps=60" => ["fps", "60"]
          // "--debug" => ["debug"]
          let parsed = arg.as_str()[2..].split("=").map(|s| s.to_string()).collect::<Vec<_>>();
  ...
  pub const fn get_fps(&self) -> u8{ self.frame.frames_per_second }
  pub const fn get_frame_time(&self) -> Duration { self.frame.frame_time }
  pub const fn get_flags(&self) -> RuntimeFlags { self.flags }
